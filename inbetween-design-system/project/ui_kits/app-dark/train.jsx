...
function App() {
  ...
  // Dark theme tokens (brand default)
  const INK = '#F7F6F3';
  const INK2 = 'rgba(247,246,243,0.8)';
  const INK3 = 'rgba(247,246,243,0.5)';
  const INK4 = 'rgba(247,246,243,0.30)';
  ...
  return (
    <div style={{
      ...
      {/* grain texture */}
      <div style={{ position: 'absolute', inset: 0, background: 'var(--grain-url, none)', backgroundSize: '220px', opacity: .4, mixBlendMode: 'overlay', pointerEvents: 'none' }} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', paddingBottom: 90 }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0 20px 12px', position: 'relative' }}>
          <button style={{
            ...
            <Bell color={INK} />
            {/* notification dot */}
            <div style={{ position: 'absolute', top: 9, right: 9, width: 6, height: 6, borderRadius: '50%', background: '#E8B530', boxShadow: '0 0 0 1.5px #000' }} />
          </button>
          <div style={{
            width: 40, height: 40, borderRadius: '50%',
            backgroundImage: 'url(avatar.png)', backgroundSize: 'cover', backgroundPosition: '62% 30%',
            boxShadow: '0 0 0 2px rgba(232,181,48,0.25), 0 0 0 3px rgba(240,194,74,0.15), 0 6px 14px -4px rgba(0,0,0,.4)',
            position: 'relative',
          }} />
        </div>

        {/* TODAY'S FOCUS card — gold gradient hero */}
        <div style={{
          margin: '0 18px 12px',
          background: 'linear-gradient(135deg, #F6D27A 0%, #E8B530 100%)',
          border: '1px solid rgba(255,255,255,0.15)',
          borderRadius: 22,
          padding: '16px 20px',
          position: 'relative',
          overflow: 'hidden',
          boxShadow: '0 0 0 1px rgba(232,181,48,.35), 0 12px 40px -10px rgba(232,181,48,.35)',
        }}>
          {/* inner highlight */}
          <div style={{ position: 'absolute', inset: 0, borderRadius: 24, pointerEvents: 'none', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.25)' }} />
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{
              ...
              padding: '5px 11px',
              background: 'rgba(0,0,0,0.25)', color: '#0A0A0A',
              borderRadius: 999,
          ...
          <div style={{ marginTop: 12, fontSize: 32, fontWeight: 600, letterSpacing: '-0.03em', lineHeight: 0.95, color: '#0A0A0A' }}>Go extreme</div>
          <div style={{ marginTop: 4, fontSize: 11, color: 'rgba(0,0,0,0.5)', letterSpacing: '.06em', textTransform: 'uppercase', fontWeight: 500 }}>1st Session · 18 min</div>

          <div style={{ marginTop: 10, fontSize: 12.5, lineHeight: 1.5, color: 'rgba(0,0,0,0.7)', fontWeight: 300, maxWidth: 320, letterSpacing: '-.005em' }}>
            Push movement to extremes, embrace mistakes, and trust your body over logic.
          </div>

          <button style={{
            width: '100%', marginTop: 12,
            background: '#0A0A0A', color: '#F7F6F3',
            border: 0, padding: '11px 20px', borderRadius: 12,
            fontFamily: 'inherit', fontWeight: 600, fontSize: 14.5, letterSpacing: '-.005em',
            display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9,
            cursor: 'pointer',
            boxShadow: '0 1px 0 rgba(255,255,255,0.1) inset, 0 -1px 0 rgba(0,0,0,0.2) inset, 0 4px 12px -4px rgba(0,0,0,.4)',
          ...
          {days.map((d, i) => {
            ...
            return (
              <button key={i} onClick={() => setDay(i)} style={{
                height: 44,
                background: selected ? '#E8B530' : TILE,
                border: selected ? '1px solid #F0C24A' : `1px solid ${LINE}`,
                borderRadius: 10,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                position: 'relative', cursor: 'pointer',
                color: selected ? '#0A0A0A' : INK2,
                fontSize: 14, fontWeight: 600, letterSpacing: '-.01em',
                fontFamily: 'inherit',
                boxShadow: selected ? '0 0 0 1px rgba(232,181,48,.35), 0 4px 12px -4px rgba(232,181,48,.45)' : 'none',
                transition: 'all 120ms cubic-bezier(.2,.7,.2,1)',
              }}>
                {d.l}
                {d.dots.length > 0 && (
            ...
            return (
              <button
                ...
                style={{
                  ...
                  flex: 1,
                  padding: '9px 12px',
                  background: active ? '#E8B530' : 'transparent',
                  color: active ? '#0A0A0A' : INK3,
                  border: 0,
                  borderRadius: 999,
                  fontFamily: 'inherit',
                  fontSize: 11,
                  ...
                  boxShadow: active ? '0 0 0 1px rgba(232,181,48,.35), 0 2px 6px rgba(232,181,48,.35)' : 'none',
                }}
              >
                {label}
              </button>
